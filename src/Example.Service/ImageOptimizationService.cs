...
public class ImageOptimizationService
{
    ...
    /// <returns></returns>
    public async Task OptimizeAsync(Stream input, Stream output, string contentType, int maxWidth = 800, int maxHeight = 600)
    {
    ...
    /// <returns></returns>
    public Task OptimizeAsync(Stream input, Stream output, string contentType, Size maxSize)
    {
        return OptimizeAsync(input, output, contentType, maxSize.Width, maxSize.Height);
    }
    ...
    private Size GetResizedSize(Size input, Size basis)
    {
        ...
        Debug.WriteLine($"Resize info: ({input.Width},{input.Height}) to ({w}, {h})");
